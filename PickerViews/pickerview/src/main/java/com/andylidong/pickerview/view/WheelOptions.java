...
public class WheelOptions<T> {
	...
	// 设置item的个数
	private int setItemsVisible = 11;

	private static int currentOption1 = -1;
	private static int currentOption2 = -1;
	private static int currentOption3 = -1;
	...
	public void setPicker(ArrayList<T> options1Items,
		...
		wv_option1.setItemsVisible(setItemsVisible);// 显示picker的显示条目
		currentOption1  = wv_option1.getCurrentItem();// 记录当前选中的位置
		// 选项2
		...
		if (mOptions2Items != null) {
			...
			wv_option2.setItemsVisible(setItemsVisible);// 显示picker的显示条目
			currentOption2  = wv_option2.getCurrentItem();// 记录当前选中的位置
		}
		// 选项3
		...
		if (mOptions3Items != null) {
			...
			wv_option3.setItemsVisible(setItemsVisible);// 显示picker的显示条目
			currentOption3  = wv_option3.getCurrentItem();// 记录当前选中的位置
		}
		...
		wheelListener_option1 = new OnItemSelectedListener() {
			...
			public void onItemSelected(int index) {
				...
				if (index >= 0) {
					currentOption1 = index;
					if (optionsSelectListener != null) {
						if (currentOption2 != -1) {
							optionsSelectListener.onOptionsSelect(currentOption1, currentOption2, currentOption3);
						} else {
							optionsSelectListener.onOptionsSelect(currentOption1);
						}
					}
				}
			}
